...
function getRanges(data: [number, number, number][]) {
    // 最大
    const xR = [data[0][0], data[0][0]]
    const yR = [data[0][1], data[0][1]]
    const vR = [data[0][2], data[0][2]]
...
function colorArrayToRGBAByteArray(data: [number, number, number][], width: number, height: number) {

    // 最大
    let xR = [data[0][0], data[0][0]]
    let yR = [data[0][0], data[0][1]]
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const rgba = valueToColor(vR[0], vR[1], value)
...
function App() {
    ...
    async function drawHeatMap() {
        ...
        const ranges = getRanges(colorData);
        const pixelData = colorArrayToRGBAByteArray(colorData, ranges.width, ranges.height)
        ...
        // 创建 PIXI.BaseTexture
        const baseTexture = PIXI.BaseTexture.fromBuffer(new Uint8ClampedArray(pixelData.buffer), ranges.width, ranges.height);
        ...
        heatmapSprite.width = ranges.width;
        heatmapSprite.height = ranges.height;
